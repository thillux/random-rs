...
fn main() {
    ...
    let opensc_engine_path = CString::new(args.pkcs11_engine_sc).expect("Conversion to CString failed for opensc");
    let tpm2_engine_path = CString::new(args.pkcs11_engine_tpm2).expect("Conversion to CString failed for tpm2 engine");

    let mut rand_buffer = [0; 256 / 8];
    unsafe {
        ...
        loop {
            ...
            println!("{rand_buffer:x?}");
            random_rs::add_kernel_entropy(i32::try_from(0).unwrap(), rand_buffer.as_ptr(), rand_buffer.len());
